...
import { createPayrollHeaderColumns } from "./columns/payroll-header-columns";

interface PayrollHeaderTableProps {
  data: PayrollHeader[];
  loading?: boolean;
  error?: string | null;
...
export function PayrollHeaderTable({
  data,
  loading = false,
  error = null,
  onViewPayroll,
  searchTerm = "",
}: PayrollHeaderTableProps) {
  ...
  const columns = useMemo(
    () => createPayrollHeaderColumns(onViewPayroll),
    [onViewPayroll]
  );
  ...
  // Update global filter when searchTerm prop changes
  useEffect(() => {
