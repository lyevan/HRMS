...
const ChangeShift = () => {
  ...
  const fetchChangeShiftRequests = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (response.success) {
        ...
        setError("Failed to fetch change shift requests");
      }
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
          : "Failed to fetch change shift requests"
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <Button onClick={() => fetchChangeShiftRequests()}>Try Again</Button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
                ...
                {loading ? (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-8">
                      Loading change shift requests...
                    </TableCell>
                  </TableRow>
                ) : filteredRequests.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={7} className="text-center py-8">
                      No change shift requests found
                    </TableCell>
                  </TableRow>
                ) : (
                      ...
                      <TableCell>
                        <div className="text-sm">
                          {request.start_date && request.end_date
                            ...
                            : request.start_date
                            ? `From ${formatRequestDate(request.start_date)}`
                            : "N/A"}
                        </div>
                      </TableCell>
      ...
      <Modal
        ...
        title="File Change Shift Request"
        description="Submit a new shift change request"
      >
        <FileChangeShiftContent
      ...
      <Modal
        ...
        title={
          detailsReadOnly
            ? "Change Shift Request Details"
            : "Review Change Shift Request"
        }
        description="View and manage shift change request"
      >
        <ChangeShiftDetailsContent
