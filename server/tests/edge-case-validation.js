...
async function testUltimateEdgeCase() {
  console.log(
    "🔥 TESTING ULTIMATE EDGE CASE: Day Off + Regular Holiday + Night Diff + Overtime"
  );
  console.log(
    "==============================================================================="
  );
  ...
  try {
    ...
    console.log(
      ...
      JSON.stringify(attendanceBreakdown, null, 2)
    );
    ...
    const aggregatedAttendance = {
      ...
      night_differential_hours:
        attendanceBreakdown.night_differential_hours || 0,
    ...
    console.log(
      "📊 Aggregated Attendance for Payroll:",
      JSON.stringify(aggregatedAttendance, null, 2)
    );
    ...
    if (Math.abs(expectedTotal - actualTotal) <= tolerance) {
      console.log(
        "✅ VALIDATION PASSED: Calculations are within acceptable tolerance!"
      );
    } else {
      console.log(
        "❌ VALIDATION FAILED: Calculations exceed tolerance threshold!"
      );
    }
...
async function testRegularDayComparison() {
  ...
  const calculator = new AdvancedPayrollCalculator();
  const regularEarnings = await calculator.calculateEarnings(
    testEmployee,
    regularAttendance,
    testSchedule
  );
  ...
  console.log(
    "📊 Ultimate Edge Case: ₱" + (await testUltimateEdgeCase()).actual
  );
  console.log(
    "📊 Premium Multiplier: " +
      (
        (await testUltimateEdgeCase()).actual / regularEarnings.grossPay
      ).toFixed(2) +
      "x"
  );
}
